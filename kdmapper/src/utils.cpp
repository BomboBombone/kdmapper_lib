...
std::wstring kdmapper_utils::GetFullTempPath() {
...
bool kdmapper_utils::ReadFileToMemory(const std::wstring& file_path, std::vector<uint8_t>* out_buffer) {
...
bool kdmapper_utils::CreateFileFromMemory(const std::wstring& desired_file_path, const char* address, size_t size) {
...
uint64_t kdmapper_utils::GetKernelModuleAddress(const std::string& module_name) {
	void* buffer = nullptr;
	DWORD buffer_size = 0;
...
BOOLEAN kdmapper_utils::bDataCompare(const BYTE* pData, const BYTE* bMask, const char* szMask) {
...
uintptr_t kdmapper_utils::FindPattern(uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, char* szMask) {
...
PVOID kdmapper_utils::FindSection(char* sectionName, uintptr_t modulePtr, PULONG size) {
